...
# When a Gemini call fails we fall back to a deterministic extractor. That
# fallback invents a location from a fixed list, so a silent fallback produces
# confident-looking but wrong records. Track it and report it instead.
_degraded: dict[str, object] = {
    "active": False,
    "reason": None,
    "since": None,
    "failures": 0,
...
def _record_degradation(source: str, exc: Exception) -> None:
    now = datetime.now(timezone.utc).isoformat()
    _degraded.update(
        active=True,
        reason=f"{source}: {type(exc).__name__}: {str(exc)[:200]}",
        since=_degraded["since"] or now,
        failures=int(_degraded["failures"]) + 1,
    )
    print(f"[{source}] Gemini call failed, serving fallback: {exc}")


def _record_success() -> None:
    _degraded.update(active=False, reason=None, since=None)
...
class Report(BaseModel):
    ...
    lat: float | None = None
    lng: float | None = None
    # "gemini" if Gemini produced this record, "mock" if the deterministic
    # fallback did — in which case the location is invented, not extracted.
    # Rows written before this column existed default to "gemini".
    extraction_mode: str = "gemini"


# The structured shape we ask Gemini to extract (no id/created_at — those are
# assigned server-side). Deliberately excludes extraction_mode: this class is
# passed to Gemini as a response schema, so any field here is one the model is
# asked to fill in.
...
def init_db():
    with get_db() as conn:
        ...
        # Migration for databases created before extraction_mode existed.
        columns = {
        ...
        if "extraction_mode" not in columns:
            conn.execute(
                ...
                "extraction_mode TEXT NOT NULL DEFAULT 'gemini'"
            )
...
def insert_report(
    data: ExtractedReport, extraction_mode: str = "gemini"
) -> Report:
        ...
        cur = conn.execute(
            ...
            INSERT INTO reports
                (created_at, name, age, urgency, title, summary, notes,
                 other_data, location, lat, lng, extraction_mode)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                created_at,
...
def extract_mock(raw_text: str) -> ExtractedReport:
    ...
    return ExtractedReport(
        ...
        other_data=(
            ...
            + (
                " because the Gemini call failed"
                if GEMINI_API_KEY
                else " because no GEMINI_API_KEY is set"
            )
            + ". The location below is a placeholder, not extracted from the "
            "report text."
        ),
        location=location,
        lat=lat,
...
def extract(raw_text: str) -> tuple[ExtractedReport, str]:
    """Return the extracted report and which extractor actually produced it."""
    if GEMINI_API_KEY:
        try:
            ...
            _record_degradation("triage", exc)
    return extract_mock(raw_text), "mock"
...
def get_support(request: SupportRequest) -> SupportResponse:
    if GEMINI_API_KEY:
        try:
            result = support_with_gemini(request)
            _record_success()
            return result
        except Exception as exc:  # noqa: BLE001 - retain key-free safe fallback
            _record_degradation("support", exc)
...
@app.get("/")
def health():
    configured = "gemini" if GEMINI_API_KEY else "mock"
    # `mode` is what the last call actually used, not merely what is configured:
    # a key can be present while every request falls back on quota errors.
    effective = "mock" if (_degraded["active"] or not GEMINI_API_KEY) else "gemini"
    return {
        "status": "ok" if not _degraded["active"] else "degraded",
        "service": "AI Crisis Triage Mapper",
        "mode": effective,
        "configured_mode": configured,
        "degraded": bool(_degraded["active"]),
        "degraded_reason": _degraded["reason"],
        "degraded_since": _degraded["since"],
        "fallback_count": _degraded["failures"],
...
def triage(request: TriageRequest):
    ...
    extracted, mode = extract(request.raw_text)
    return insert_report(extracted, mode)
